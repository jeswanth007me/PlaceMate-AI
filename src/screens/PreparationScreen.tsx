...
export const PreparationScreen: React.FC = () => {
  ...
  if (activeSession) {
    ...
    // 1. Candidate's weak areas from interview
    if (activeSession.weakAreas && activeSession.weakAreas.length > 0) {
      activeSession.weakAreas.forEach((weakness, idx) => {
        personalizedTopics.push({
          id: `weakness-${idx}`,
          topic: weakness,
          explanation: `Identified gap from your recent ${activeSession.interviewType} session for ${activeSession.targetRole}.`,
          keyConcepts: [
            'Systematically validate assumptions and edge cases before coding.',
            'Address performance bottlenecks, time/space trade-offs, and scalability constraints.',
            'Formulate invariants explicitly when discussing your architectural decisions.',
          ],
          retrievedMaterial: `Interview Evaluation Gap: Review fundamentals and common interview questions related to "${weakness}".`,
          relevantWeakness: weakness,
        });
      });
    } else {
      activeSession.evaluations.forEach((ev, idx) => {
        if (ev.improvements && ev.improvements.length > 0) {
          const weakness = ev.improvements[0];
          personalizedTopics.push({
            id: `weakness-${idx}`,
            topic: ev.retrievedPrepMaterial?.topic || weakness,
            explanation: ev.retrievedPrepMaterial?.summary || `Focus area identified during question evaluation.`,
            keyConcepts: [
              ev.retrievedPrepMaterial?.keyConcept || 'Validate complexity trade-offs explicitly.',
              'Quantify data access patterns and latency requirements.',
            ],
            retrievedMaterial: `Interview Feedback: ${weakness}`,
            relevantWeakness: weakness,
          });
        }
      });
    ...
    // 2. Candidate's identified skills
    if (user.skills && user.skills.length > 0) {
      user.skills.slice(0, 3).forEach((skill, idx) => {
        personalizedTopics.push({
          id: `skill-${idx}`,
          topic: `${skill} Deep Dive`,
          explanation: `Advanced concepts in ${skill} tailored for senior interview benchmarks.`,
          keyConcepts: [
            `Internals, concurrency primitives, and memory model in ${skill}.`,
            `Standard design patterns and profiling tooling in ${skill}.`,
          ],
          retrievedMaterial: `Candidate Skill Context: Verified skill from your profile (${skill}). Expected interview depth includes internals and distributed systems interaction.`,
          relevantWeakness: 'Advanced mastery',
        });
      });
    }

    // 3. Target role preparation
    if (user.targetRole || activeSession.targetRole) {
      const role = user.targetRole || activeSession.targetRole;
      personalizedTopics.push({
        id: 'target-role',
        topic: `${role} Interview Mastery`,
        explanation: `Core competencies and architectural expectations for ${role} roles.`,
        keyConcepts: [
          'High-level architecture, service decomposition, and interface design.',
          'Production reliability: observability, circuit breakers, and fault tolerance.',
        ],
        retrievedMaterial: `Role Benchmark: Industry standards for ${role} placement interviews.`,
        relevantWeakness: 'Role readiness',
      });
    }
  }
